...
varying vec4 v_position; // The local position of the geometry
...
void main() {
    ...
    // Lights
    float NdotL= dot(geo.normal, directionalLights[0].direction);
    float lightIntensity = smoothstep(0.0, 0.01, NdotL);
    vec3 directionalLight = directionalLights[0].color * lightIntensity;

    

    vec3 halfVector = normalize(directionalLights[0].direction + geo.viewPos);
    ...
    vec3 outLightColor = directionalLight + specular + ambientLightColor;
